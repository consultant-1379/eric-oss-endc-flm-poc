...
public class PmRopGUtranFreqRelation implements PmRop, Serializable {
    @Serial
    ...
    private double pmEndcDistrFreqSel;

    @Override
    public boolean fromAvroObject(GenericRecord avroObject) {
        if (avroObject.getSchema().getName().equals("GUtranFreqRelation_1")) {
            ...
            try {
                ...
                log.error("Failed to set PmCounters for PmRopGUtranFreqRelation: {}", pmCounters.toString(), e);
                return false;
            }
        ...
        log.error("Failed to parse PmRopGUtranFreqRelation: incorrect object type: {}", avroObject.toString());
