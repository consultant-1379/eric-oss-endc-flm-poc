...
import com.ericsson.oss.apps.repository.PmGNBDUFunctionRepo;
...
public class GNBDUFunctionPmCounterDeserializerIntegrationTest extends PmCounterDeserializerIntegrationTestBase<LtePmCounterDataRetrieval> {
    ...
    @Captor
    private ArgumentCaptor<List<PmRopGNBDUFunction>> listArgumentCaptor;
    @Autowired
    ...
    private static final String MO_NAME = "GNBDUFunction_GNBDU";
    ...
    GenericRecord buildRecordData() {
        ...
        try {
            schemaNRCellCU = new Schema.Parser().parse(new File("./src/test/resources/__files/schema/GNBDUFunction.avsc"));
        } catch (IOException e) {
    ...
    void validateResult() {
        ...
        PmRopGNBDUFunction pmRopGNBDUFunction = (PmRopGNBDUFunction) captorValue.get(0);
        assertEquals(FDN, pmRopGNBDUFunction.getMoRopId().getObjectId().toString());
        assertEquals(ROP_END_TIME_IN_EPOC, pmRopGNBDUFunction.getMoRopId().getRopTime());
        assertEquals(COUNTER_VALUE, pmRopGNBDUFunction.getPmMacVolUl());

        assertEquals(1, pmGNBDUFunctionRepo.findAll().size());
    }
}
