...
import com.ericsson.oss.apps.repository.PmGUtranFreqRelationRepo;
...
public class FreqRelationPmCounterDeserializerIntegrationTest extends PmCounterDeserializerIntegrationTestBase<LtePmCounterDataRetrieval> {
    ...
    @Captor
    private ArgumentCaptor<List<PmRopGUtranFreqRelation>> listArgumentCaptor;
    @Autowired
    ...
    private static final String MO_NAME = "GUtranFreqRelation";
    ...
    GenericRecord buildRecordData() {
        ...
        Schema schemaFreqRelation;
        try {
            schemaFreqRelation = new Schema.Parser().parse(new File("./src/test/resources/__files/schema/GUtranFreqRelation.avsc"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        final Schema schemaPmCounters = schemaFreqRelation.getField("pmCounters").schema();
        GenericRecord avroPmCounters = new GenericData.Record(schemaPmCounters);
        GenericRecord record = new GenericData.Record(schemaFreqRelation);
    ...
    void validateResult() {
        ...
        PmRopGUtranFreqRelation gUtranFreqRelation = (PmRopGUtranFreqRelation) captorValue.get(0);
        assertEquals(FDN, gUtranFreqRelation.getMoRopId().getObjectId().toString());
        assertEquals(ROP_END_TIME_IN_EPOC, gUtranFreqRelation.getMoRopId().getRopTime());
        assertEquals(COUNTER_VALUE, gUtranFreqRelation.getPmEndcDistrFreqSel());
        assertEquals(1, pmGUtranFreqRelationRepo.findAll().size());
    }
}
